...
def getqrcode(open_id, auth_header):
    ...
    try:
        response = requests.get(api.user_qrcode_url, params=query_params, headers=headers)
...
def getcompany(phone, pwd):
    ...
    try:
        response = requests.post(api.user_company_url, headers=headers, data=json.dumps(ret))
        response.raise_for_status()
        status_code = response.status_code
        if status_code == 200:
            return status_code, msg.msg_success, response.json()
